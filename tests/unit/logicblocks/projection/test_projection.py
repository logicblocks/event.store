...
generic_event = (StoredEventBuilder()
...
class TestProjection(unittest.TestCase):
    ...
    def test_projection_with_one_event(self):
        ...
        event = (generic_event
                 .with_name("something_occurred")
                 .with_occurred_at(occurred_at)
                 .build())
        projection = MyTestProjection()

        expected_projection = {
            "something_occurred_at": occurred_at
        }

        self.assertEqual(expected_projection,
                         projection.project({}, [event]).projection)

    def test_projection_with_two_events(self):
        something_occurred_at = datetime.now(UTC)
        something_event = (generic_event
                           .with_name("something_occurred")
                           .with_occurred_at(something_occurred_at)
                           .build())

        something_else_occurred_at = datetime.now(UTC)
        something_else_event = (generic_event
                                .with_name("something_else_occurred")
        ...
        self.assertEqual(expected_projection,
                         projection.project({}, [something_event,
                                                 something_else_event]).projection)
